...
for idx in turb_galaxies:
    ...
    ax[0, 0].imshow(struct_dens, cmap="Blues_r", norm="log", interpolation=None, origin="lower")
    ax[0, 0].set_yticks([])
    ax[0, 0].set_xticks([])
    ax[0, 0].streamplot(X, Y, U_bulk, V_bulk, density=3, color=(0, 0, 0, 0.3), arrowsize=0, linewidth=0.5, broken_streamlines=False)
    ax[0, 1].imshow(struct_dens, cmap="Blues_r", norm="log", interpolation=None, origin="lower")
    ax[0, 1].set_yticks([])
    ax[0, 1].set_xticks([])
    ax[0, 1].streamplot(X, Y, U_turb, V_turb, density=3, color=(0, 0, 0, 0.3), arrowsize=0, linewidth=0.5, broken_streamlines=False)
    ax[0, 1].plot([struct_dens.shape[0]*(1 - 1/40) - scalebar_pixels, struct_dens.shape[0]*(1 - 1/40)], [struct_dens.shape[0]*(1 - 1/40), struct_dens.shape[0]*(1 - 1/40)], color='white', linewidth=1)
    ax[0, 1].text(struct_dens.shape[0]*(1 - 1/40) - scalebar_pixels/2, struct_dens.shape[0]*(1 - 1/30), f"{scalebar_length} kpc", ha="center", va="top", color="white")


    coords[:, [1, 2]] = coords[:, [2, 1]]
    bulk_magnetic[:, [1, 2]] = bulk_magnetic[:, [2, 1]]
    turb_magnetic[:, [1, 2]] = turb_magnetic[:, [2, 1]]
    grid_size = 1000
    grid = (range(0, grid_size), range(0, grid_size), range(grid_size//2, grid_size//2 + 1))
    struct_bulk = nicegalaxy.map_unstructured_to_structured_slice_optimized(coords, bulk_magnetic, grid_size=grid_size, grid=grid)
    Y, X = np.mgrid[0:grid_size, 0:grid_size]
    ...
    scalebar_length = round(cutout_size/10)
    scalebar_pixels = scalebar_length/cutout_size * struct_dens.shape[0]
    ax[1, 0].imshow(struct_dens, cmap="Blues_r", norm="log", interpolation=None, origin="lower")
    ax[1, 0].set_yticks([])
    ax[1, 0].set_xticks([])
    ax[1, 0].streamplot(X, Y, U_bulk, V_bulk, density=3, color=(0, 0, 0, 0.3), arrowsize=0, linewidth=0.5, broken_streamlines=False)
    ax[1, 1].imshow(struct_dens, cmap="Blues_r", norm="log", interpolation=None, origin="lower")
    ax[1, 1].set_yticks([])
    ax[1, 1].set_xticks([])
    ax[1, 1].streamplot(X, Y, U_turb, V_turb, density=3, color=(0, 0, 0, 0.3), arrowsize=0, linewidth=0.5, broken_streamlines=False)


    fig.tight_layout()
    fig.savefig(f"turb-magnetic-images-collage/{idx}.png", bbox_inches="tight", dpi=500)
